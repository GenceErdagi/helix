...
impl Tree {
    ...
    pub fn resize_view(&mut self, direction: Direction, count: usize) {
        ...
        if let Some((container, idx)) = self.find_container(layout) {
            ...
            let diff = match direction {
                Direction::Left | Direction::Right => 1.0 / container.area.width as f64 * 2.0,
                Direction::Up | Direction::Down => 1.0 / container.area.height as f64 * 2.0,
            };
            let diff = diff * count as f64;

            let b = if idx == container.children.len() - 1 {
                idx - 1
            } else {
                idx
            };

            let moving_left_or_up = matches!(direction, Direction::Left | Direction::Up);
            
            let bounds = &mut container.children.bounds;
            let min = 0.05;
            let current_b = bounds[b].portion;
            let current_next = bounds[b+1].portion;
            
            let (new_b, new_next) = if moving_left_or_up {
                let actual_diff = f64::min(diff, current_b - min);
                if actual_diff > 0.0 {
                    (current_b - actual_diff, current_next + actual_diff)
                } else {
                    (current_b, current_next)
                }
            } else {
                let actual_diff = f64::min(diff, current_next - min);
                if actual_diff > 0.0 {
                    (current_b + actual_diff, current_next - actual_diff)
                } else {
                    (current_b, current_next)
            ...
            bounds[b].portion = new_b;
            bounds[b+1].portion = new_next;
            self.recalculate();
        }
    }
